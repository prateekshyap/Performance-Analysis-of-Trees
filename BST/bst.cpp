...
#include"nodesBST.h" // contains all the node structures
#include"datastructuresBST.h" //contains all the data structures
#include"functionsBST.h" //contains functions

extern int bstFileCount, bstFileType; //external variables
...
int main(int argc, char ** argv)
{
	...
	BinarySearchTree tree; //tree object
	...
	while (sequence >> tempString)
	{
		...
		while (inputFile >> inputString)
		{
			...
			{
				outputFile << "Printing treap with file name- " << printFileName << bstFileCount << ".png" << endl;
				outputFile << "Graphviz file can be found with file name- " << printFileName << bstFileCount << ".gv" << endl << endl;
				tree.printTree(printFileName.c_str()); //call print tree function
			}
			else if (inputString.compare("quit") == 0 || inputString.compare("exit") == 0 || inputString.compare("stop") == 0 || inputString.compare("return") == 0)
			{
				...
				if (inputString.compare("Y") == 0 || inputString.compare("y") == 0)
					generatePNGInBST(bstFileType); //generate images
				return 0;
			}
			else
			{
